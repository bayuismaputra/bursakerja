<div class="col-md-10 main-container">
    <div class="container">
        ...
        ?>
                ...
                $na = 0;
                $k = $kuota;
                $arr = array_reverse($nilai_v);
                for ($i = 0; $i < count($arr); $i++) {
                    $status = 0;

                    $qryCustom = $bursaKerja->queryCustom("SELECT * FROM ranking WHERE id_pelamar={$arr[$i]['id_pelamar']} AND id_lowongan={$id_lowongan}");
                    ...
                    if ($qryCustom->rowCount() > 0) {
                        $q = $bursaKerja->queryCustom("UPDATE `ranking` SET `nilai_akhir` = '{$arr[$i]['nilai']}', `status` = '{$status}' WHERE `id_pelamar`={$arr[$i]['id_pelamar']} AND `id_lowongan`={$id_lowongan}");
                    } else {
                        $q = $bursaKerja->queryCustom("INSERT INTO `ranking`(`id_pelamar`, `id_lowongan`, `nilai_akhir`,`status`) VALUES ({$arr[$i]['id_pelamar']},{$id_lowongan},{$arr[$i]['nilai']},{$status})");
                    }
                }
                // var_dump($arr);
                // die;
...
<?php
            } else if ($_GET['action'] == 'konfirmasi') {
                $q = $bursaKerja->queryCustom("UPDATE `ranking` SET `status` = '1' WHERE `id_ranking`={$_GET['id']}");

                $cekS = $bursaKerja->queryCustom("SELECT * FROM `ranking` WHERE `id_lowongan`={$_GET['id_lowongan']} AND `status`='-'");
                foreach ($cekS as $row) {
                    $lowongan->queryCustom("UPDATE `ranking` SET `status` = '0' WHERE `id_ranking`={$row['id_ranking']}");
                }
?>
    <!-- rekomendasi -->
    <div class="content">
